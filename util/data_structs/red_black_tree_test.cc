...
TEST_F(RedBlackTreeTest, TestEmpty) {
  ElementTree tree;
  EXPECT_EQ(tree.LowerBound([](const Element&) {
    return true;
  }),
            nullptr);
...
TEST_F(RedBlackTreeTest, TestSingle) {
  ...
  EXPECT_EQ(tree.LowerBound([](const Element& element) {
...
TEST_F(RedBlackTreeTest, TestInsertMany) {
  ...
  for (size_t i = 0; i < kNumElements; i++) {
    Element* element = tree.LowerBound([i](const Element& element) {
...
TEST_F(RedBlackTreeTest, TestDeleteMany) {
  ...
  for (size_t i = 0; i < kNumElements; i++) {
    ...
    for (size_t j = 0; j < kNumElements; j++) {
      ...
      Element* find_j = tree.LowerBound([j](const Element& element) {
